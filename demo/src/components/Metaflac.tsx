...
export default function Metaflac() {
  ...
  const [args, setArgs] = useState(
    '--show-tag=TITLE --show-tag=ARTIST --show-tag=ALBUM input.flac'
  )
  ...
  const handleFileNameInput: JSX.GenericEventHandler<HTMLInputElement> = (
    event,
  ) => {
  ...
  return (
    <div class="container flex flex-col items-center">
      <div class="mb-5">
        You can run the official FLAC tool <code>metaflac</code>{' '}
      ...
      {outputFile && (
        <a
          href={outputFile}
          download={fileName}
          class="mt-8 button is-info is-light"
        >
          Download audio file
        </a>
      )}
